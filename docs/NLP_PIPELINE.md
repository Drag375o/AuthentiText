...
**Headings are detected but not dropped.** A paragraph that is a single short line (12 words or fewer) with no sentence-ending punctuation is marked as a heading. Headings count as sentences everywhere, including every sentence-length measure, so nothing is silently excluded from the numbers. They are also counted separately (`heading_count`) and the interface says "includes 2 headings" beside the rhythm figures, because a three-word heading legitimately pulls the mean down: in testing, one "3.3 Data Split" heading shifted burstiness by 0.16.
